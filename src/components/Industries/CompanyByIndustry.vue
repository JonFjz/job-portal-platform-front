...
<template>
  <DefaultLayout>
    <div class="container mx-auto py-8 dark:bg-gray-800">
      <h2 class="text-2xl font-bold mb-12 text-center text-gray-600 dark:text-gray-100">Browse Companies by Industry</h2>
      <div class="grid grid-cols-1 md:grid-cols-3 gap-8">
        ...
        >
          <span class="absolute inset-x-0 bottom-0 h-2 bg-green-500 dark:bg-green-600"></span>
          <div class="my-4 text-center">
            <div class="icon-container text-5xl mb-4 text-cyan-900 dark:text-gray-100">
              <i :class="getIconForIndustry(industry)"></i>
            </div>
            <h3 class="industry-name text-cyan-900 dark:text-gray-100 text-xl font-semibold">{{ industry }}</h3>
          </div>
          <div class="flex justify-center mt-6">
            <button class="px-4 py-2 text-cyan-900 dark:text-gray-100 border border-green-500 dark:border-green-600 font-semibold rounded hover:bg-green-500 hover:text-white dark:hover:bg-green-600">
              See Companies
            </button>
          </div>
        </div>
      </div>
      <div v-if="displayedIndustries.length < totalIndustries" class="flex justify-center mt-12">
        <button @click="loadMoreIndustries" class="reset-button py-3 px-6 bg-cyan-900 text-gray-100 rounded-lg hover:bg-cyan-700 transition-colors duration-300 dark:bg-cyan-800 dark:hover:bg-cyan-700">
          See all industries
        </button>
      </div>
    </div>
