<script setup>
  ...
  const fetchEmployerId = async () => {
    try {
      const response = await axios.get('https://localhost:7136/api/Employers/current')
  ...
  const fetchJobs = async () => {
    try {
      ...
      const response = await axios.get(`https://localhost:7136/api/JobPostings/by-employer/${employerId.value}`)
  ...
  const deleteJob = async (id) => {
    try {
      await axios.delete(`https://localhost:7136/api/jobpostings/${id}`)
      fetchJobs()
    } catch (err) {
        ...
        <tbody>
          <tr v-for="job in jobs" :key="job.id">
            ...
            <td class="py-2 px-4 border-b">
              <RouterLink :to="`/employer-dashboard/job-listings/${job.id}/applications`" class="bg-blue-600 text-white px-2 py-1 rounded-md hover:bg-blue-700">
                View Applications
              </RouterLink>
              <button @click="editJob(job)" class="bg-yellow-600 text-white px-2 py-1 rounded-md hover:bg-yellow-700 ml-2">Edit</button>
              <button @click="deleteJob(job.id)" class="bg-red-600 text-white px-2 py-1 rounded-md hover:bg-red-700 ml-2">Delete</button>
            </td>
          </tr>
        </tbody>
      </table>
